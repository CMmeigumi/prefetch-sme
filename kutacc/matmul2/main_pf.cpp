...
#include "bgemm_woqs8_kernel_32_pf.h"
...
{
    // Default parameters - use 32x32 block size for 32x32 kernel path
    const int m = 1024;
    const int n = 1024;
    const int k = 1024;
    ...
    printf("BGEMM 32x32 Kernel Test (With Prefetch)\n");
    ...
    for (int repeat = 0; repeat < repeat_count; repeat++) {
        ...
        kutacc::bgemm_woqs8_enable_matrix_pf<true>(
            m, n, k,
            A, m,      // A: int8_t, lda = m
            B, k,      // B: bf16, ldb = k
            C, m,      // C: bf16, ldc = m
