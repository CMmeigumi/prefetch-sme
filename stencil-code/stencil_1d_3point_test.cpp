// 1D 3-point Stencil 正确性对比测试

#include <iostream>
#include <iomanip>
#include "stencil_1d_3point.h"

using namespace std;

const double TOLERANCE = 1e-10;

struct CompareResult {
    double maxDiff;
    int mismatchCount;
    int totalPoints;
    bool passed;
};

CompareResult compareGrids1D(double* grid1, double* grid2, int size) {
    CompareResult result = {0.0, 0, 0, true};

    for (int i = 1; i < size - 1; i++) {
        result.totalPoints++;
        double diff = fabs(grid1[i] - grid2[i]);
        if (diff > result.maxDiff) result.maxDiff = diff;
        if (diff > TOLERANCE) {
            result.mismatchCount++;
            result.passed = false;
        }
    }
    return result;
}

void printSamplePoints(double* grid, const string& label, int size) {
    cout << "  " << label << " 采样点 (前10个内点):" << endl;
    int count = 0;
    for (int i = 1; i < size - 1 && count < 10; i++) {
        cout << "    [" << i << "] = " << fixed << setprecision(6) << grid[i] << endl;
        count++;
    }
}

void printCompareResult(const CompareResult& result, const string& testName) {
    cout << "[" << testName << "] " << (result.passed ? "PASS" : "FAIL") << endl;
    cout << "  总计算点数: " << result.totalPoints << endl;
    cout << "  最大差异: " << scientific << result.maxDiff << endl;
    cout << "  不匹配点数: " << result.mismatchCount << " / " << result.totalPoints << endl;
    cout << "  容差阈值: " << scientific << TOLERANCE << endl;
}

int main() {
    cout << "========================================" << endl;
    cout << " 1D 3-point Stencil 正确性对比测试" << endl;
    cout << "========================================" << endl << endl;

    const int SIZE = 64;
    size_t grid_size = SIZE;

    double* g1 = (double*)aligned_alloc(64, grid_size * sizeof(double));
    double* g2 = (double*)aligned_alloc(64, grid_size * sizeof(double));
    double* g3 = (double*)aligned_alloc(64, grid_size * sizeof(double));
    double* g4 = (double*)aligned_alloc(64, grid_size * sizeof(double));

    cout << "数组大小: " << SIZE << endl << endl;

    initializeGrid1D(g1, SIZE);
    initializeGrid1D(g3, SIZE);

    stencil1D_3point_scalar(g1, g2, SIZE, 1);
    stencil1D_3point_sme(g3, g4, SIZE, 1);

    cout << "【标量版本计算后】" << endl;
    printSamplePoints(g2, "标量", SIZE);
    cout << "  平均: " << fixed << setprecision(6) << computeAverage1D(g2, SIZE) << endl << endl;

    cout << "【SME版本计算后】" << endl;
    printSamplePoints(g4, "SME", SIZE);
    cout << "  平均: " << fixed << setprecision(6) << computeAverage1D(g4, SIZE) << endl << endl;

    CompareResult r = compareGrids1D(g2, g4, SIZE);
    printCompareResult(r, "单次迭代");

    free(g1); free(g2); free(g3); free(g4);
    return r.passed ? 0 : 1;
}
